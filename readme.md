...
1. 开课吧。。。 
2. nodejs 看到cookie,session以及redis数据库了，还没搞得特别清楚。。。
...
- Promise的.then本身是同步的操作，异步的是里面的回调
...
1. leetcode周日 做出了两题，感觉第三题要做出来了，还是不行。
2. 开课吧预习内容的BOM完成
...
1. 这段时间就是一直在弄开课吧的课程内容以及React底层源码，算法以及红宝书也在看，继续积累沉淀一段时间吧。。。。。。



#### 11月11日

1. ES6类尽量在原型上定义函数。如果在原型上的函数里面又定义函数，则需要注意，最内层的函数的this指向，function(){}定义的this并不是指向类。箭头函数应该还是指向类。